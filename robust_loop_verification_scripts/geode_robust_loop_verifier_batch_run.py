...
def main() -> int:
    ...
    for plan in plans:
        ...
        try:
            ...
            metric_batch_rows, merged_metrics = _metrics_with_selected_score_sweep(
                plan,
                metrics,
            )
            row["status"] = "ok"
            row["metrics"] = merged_metrics
            metric_rows.extend(metric_batch_rows)
...
def _metrics_with_selected_score_sweep(
    plan: SequencePlan,
    ...
    candidate_records = plan.run_root / "candidate_records.jsonl"
    if candidate_records.is_file():
        score_sweep = compute_score_sweep(read_candidate_records(candidate_records))
        score_sweep_root = plan.run_root / "score_sweep"
        score_sweep_root.mkdir(parents=True, exist_ok=True)
        write_json(score_sweep_root / "score_sweep.json", score_sweep)
        write_score_sweep_markdown(score_sweep_root / "score_sweep.md", score_sweep)
        score_rows = {row["name"]: row for row in score_sweep["scores"]}
        for method in SELECTED_SCORE_SWEEP_METHODS:
            row = score_rows.get(method)
            if row is None:
                continue
            merged_metrics[method] = {
                "AP": float(row["AP"]),
                "MR@100P": float(row["MR@100P"]),
            }
    return _flatten_metrics(plan, merged_metrics), merged_metrics
