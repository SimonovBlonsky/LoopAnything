...
from baseline_scripts.score_boq_rover_aligned_benchmark import (
    BoQConfig,
    BoQDescriptorResult,
    BoQImageDescriptor,
...
def test_score_benchmark_pairs_preserves_pair_order_and_reuses_unique_image_descriptors(
    ...
    assert rows[0]["resized_height_query"] == 322
    assert rows[0]["resized_width_candidate"] == 322
...
def test_boq_descriptor_prepare_fails_fast_for_missing_repo(tmp_path):
    descriptor = BoQImageDescriptor(
        boq_root=tmp_path / "missing_boq",
    ...
    try:
        ...
        assert "BoQ repository not found" in str(exc)
    else:
        raise AssertionError("expected missing BoQ repository to fail before scoring pairs")
